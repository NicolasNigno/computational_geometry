...
#Autores: Julián Mauricio Flórez

def getValid_points(array, center):
    valid_points = [list(center)]
    for point in valid_points:
        for one in array:
            if (cityblock(np.array(point), one) <= 1) & (list(one) not in valid_points):
                valid_points.append(list(one))
...
while True:
    ...
    for cont in contornos:
        ...
        if area > 1000: #visualiza los contornos con areas mayores
            ...
            idx = np.random.randint(len(ones), size=int(len(ones)*0.01))
            valid_points = ones[idx,:]
            #valid_points = getValid_points(ones, np.array([centroX,centroY]))
            points_c, vertices_c, simplices_C, points_simplices_c = convexhull_nd(valid_points)
            
            pts = np.flip(vertices_c, 1).reshape((-1,1,2))
            convex = cv2.polylines(frame,[pts],True,(0,255,255), thickness=5)

    cv2.imshow("area de zeros", imgMask)
    cv2.imshow("mask", mask)
    cv2.imshow("convexhull", convex)
